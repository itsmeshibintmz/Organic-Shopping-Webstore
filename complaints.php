...
$viewcomplaints="SELECT DISTINCT tbl_complaints.* , tbl_product.name FROM tbl_complaints ,tbl_product,tbl_customer_order where tbl_complaints.email='email' and tbl_complaints.stock_product_id=tbl_customer_order.stock_product_id and tbl_customer_order.stock_product_id=tbl_product.pid  and tbl_complaints.status='NEW'";

$d_seller_prod=mysqli_query($con,$viewcomplaints);
$disp="SELECT  tbl_login.email, tbl_login.user_type_id,tbl_registration.fname,tbl_registration.lname,tbl_registration.phone_no FROM tbl_login INNER JOIN  tbl_registration
ON tbl_login.email=tbl_registration.email WHERE  tbl_login.email='email'";
